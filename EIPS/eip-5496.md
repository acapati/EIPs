...
## Abstract
This specification defines standard functions that extend the existing [EIP-721](./eip-721.md) standard, providing sharable, multiple privileges for an NFT collection. The privileges could be 1) on-chain attributes, such as voting rights, or to claim a targeted airdrop; 2) or off-chain attributes, such as a coupon for an online store, or a special offer from a local New York restaurant, or even VIP lounge access at the airport.  There could be multiple privileges attached to one NFT and each privilege holder of an NFT can transfer the privilege to others. The ownership of the privileges is verifiable. On top of the multiple-privilege extension, this standard also contains methods to give the provider, specifying an arbitrary privilege as  non-sharable or sharable. Sharable privileges can be done by simply cloning a privilege, furthermore, the provider would be able to adjust the details of privileges according to the spreading path. Expiration period can also be set to each privilege. The proposal depends on and extends the existing [EIP-721](./eip-721.md).

## Motivation
Many NFTs have functions other than just being used as a profile picture or collection of artwork, they may have real utilities in different scenarios. For example, a fashion store may give an exclusive offer for its own NFT holders; a DAO member NFT holder can vote for the proposal of how to use their treasury; some dApp may create an airdrop event to attract a certain group of people like some bluechip NFT holders to claim; a grocery store chain can issue its membership card on chain(NFT) and give certain privileges when members shop at its stores, and so on. There are cases when people who own NFTs do not necessarily want to or cannot use associated privileges. By providing additional data recording different privileges a NFT collection has and an interface for management, users would be able to transfer or sell privileges without losing their ownership of the NFT.
